...
softLLTracing = 1;                          % 是否使用 software LL 追跡方式
...
pupilSize_PLArray = [65];                   % pupil radius (mm) % note: coding PS II (必未分段)

PS_array_BLPROI = [10];                     % 建立 BLPROI 的 PS (通常設定成 VZawPS)。 需與 VD 相同數量
% --------------------------------------------------------- %
%% System Module %%
% --------------------------------------------------------- %
% upper part (prism)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function data = LRoV_II_BLP_Process(data) 
    
    %% EI 數量 check
    % 因為 BLP ROI 要綁定相鄰 Lens，最左和最右的 BLP 線會有誤差
    % 只跑一個 EI: 全部都是有誤的
    EIHor = data.EIHor;
    if ~isequal(EIHor,999) && length(EIHor) == 1;ThrowError("II",8);end

    %% preprocessing before any loop
    PS_array_BLPROI = data.PS_array_BLPROI; % 先獨立出來
    data = pre_before_any_loop("II",data); 

    %% 紀錄初始分段情形
    segmentMode_original = data.segmentMode;
    ...
    for whichFile = 1:totalNumFile 
        ...
        if size(WD_PLArray,2) ~= size(PS_array_BLPROI,2); ThrowError("II",7);end
        ...
        PS_BLPROI = PS_array_BLPROI(1,WD_num); % 獨立出來
%         structvars(5,data)
        
        %% output filename
        outputIIName = display_setup("II",data,"BLP");                data.outputIIName = outputIIName;
%         structvars(5,data)
        ...
        % 紀錄 VZawPS RP for BLP ROI
        segM00 = getSegM00(data,2,PS_BLPROI);         % 1: II mode (PS0), 2: for BLP ROI(VZawPS)       
        if isequal(segM00.M00Parameter_Matrix,-1);ThrowError("II",6);end
        data.segM00 = segM00;
        % create BLP ROI
        [midLineCell,LLLineCell] = createBLPROI(data);
        disp("<< BLP ROI 建立完成 >>")
        disp(strcat("花費時間: ",num2str(toc(tBLPROI)),"s"));
        disp('-------------------------------------------');
        % 以上只為了 midLineCell, LLLineCell 兩變數
        
        %% 分段改為"未分段"重新計算
        % for large PS
        disp('-------------------------------------------');
        disp("<< 建立 II >>")
        data.segmentMode = 0; % large PS 必須為未分段
        data = pre_before_any_loop("II",data,0);    % 第二次進入不需要選圖
        segM00 = getSeg("II",data);data.segM00 = segM00;
        data.imageFilepath = imageFilepath; data.originalImage = originalImage;
        data.WD = WD; data.thetaPolarAngle = thetaPolarAngle; data.phiAzimuthalAngle = phiAzimuthalAngle;
        data.pupilSize = pupilSize; data.RDP = RDP; data.RDP_original = RDP_original;
        data.numII = numII;
        data.outputIIName = outputIIName;
        data = pre_in_parameter_loop("II",data);
        if debugMode == 0
            % 紀錄 M00 (仍然使用 PS0)
            segM00 = getSegM00(data,1);     % 1: II mode (PS0), 2: for BLP ROI(VZawPS) 
            if isequal(segM00.segmentPoint_Matrix,-1) || isequal(segM00.M00Parameter_Matrix,-1);ThrowError("II",7);end
            data.segM00 = segM00;
        end
        ...
        if TIRHappen == 1 % LPS 發生 TIR
            outputIIName_update = erase(outputIIName,".png");
            outputIIName_update = strcat(outputIIName_update,"_LPS(TIR).png");
...
%         structvars(5,data)
        ...
        disp('-------------------------------------------');
        disp("< mapping.m 執行開始 >")
        II = func_mapping("II",data,"BLP",midLineCell,LLLineCell);
        disp("< mapping.m 執行完成 >")
        ...
        %% segmentMode 要回到初始值
        data.segmentMode = segmentMode_original;
        segM00 = getSeg("II",data);
        data.segM00 = segM00;
